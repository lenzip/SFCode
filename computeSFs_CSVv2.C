...
void computeSFs_CSVv2(const char* filein, const char* wp, const char* outdir)
{
   gROOT->SetBatch();
   ...
   for (unsigned ibin = 0; ibin < 9; ++ibin){
     ...
     cf->SetDataTag(Form("data/JP__ptbin_%s_CombIVF%s", thebin, wp));
     cf->SetDataUntag(Form("data/JP__ptbin_%s_CombIVF%s_Fail", thebin, wp));
     ...
     cf->AddTemplateTag("b",Form("mc/JP__ptbin_%s_b_CombIVF%s", thebin, wp),2);
     cf->AddTemplateTag("c",Form("mc/JP__ptbin_%s_c_CombIVF%s", thebin, wp),3);
     cf->AddTemplateTag("l",Form("mc/JP__ptbin_%s_l_CombIVF%s", thebin, wp),4);

     cf->AddTemplateUntag("b",Form("mc/JP__ptbin_%s_b_CombIVF%s_Fail", thebin, wp),2);
     cf->AddTemplateUntag("c",Form("mc/JP__ptbin_%s_c_CombIVF%s_Fail", thebin, wp),3);
     cf->AddTemplateUntag("l",Form("mc/JP__ptbin_%s_l_CombIVF%s_Fail", thebin, wp),4);   
     ...
     for (unsigned int ifit = 0; ifit < 7; ++ifit){
       ...
       if (ifit == 4) { //use MC JP calib
        ...
        cf->SetDataTag(Form("data_inverted/JP__ptbin_%s_CombIVF%s", thebin, wp));
        cf->SetDataUntag(Form("data_inverted/JP__ptbin_%s_CombIVF%s_Fail", thebin, wp));
       } 
       if (ifit < 5){ 
         if (ifit == 4)
           system("mkdir pics");
         ...
         double nbmctag = ((TH1*)fileinraw.Get(Form("mc/JP__ptbin_%s_b_CombIVF%s", thebin, wp)))->Integral() ;
         double nbmctag_jp = nbmctag - ((TH1*)fileinraw.Get(Form("mc/JP__ptbin_%s_b_CombIVF%s_JP0", thebin, wp)))->Integral();
   ...
   TFile output(Form("CombIVF%s.root", wp), "recreate"); 
   ...
   system(Form("mv CombIVF%s.root %s/%s", wp, outdir, wp));
