...
double computeSF(CFIT::cfit *cf, bool debug=false){
   float par[100];
   float err[100];
   float par_tag[100];
   float err_tag[100];
...
void computeSFs(const char* filein, const char* wp, const char* outdir)
{
   ...
   for (unsigned ibin = 0; ibin < 9; ++ibin){
     ...
     cf->AddSys("bSV","_bSV_do","_bSV_up");
     cf->AddSys("cSV","_cSV_do","_cSV_up");
     cf->AddSys("lSV","_lSV_do","_lSV_up");
     ...
     cf->SetData(Form("data/SVmass__ptbin_%s", thebin));
     cf->SetDataTag(Form("data/SVmass__ptbin_%s_DeepCSVBDisc%s", thebin, wp));
     cf->SetDataUntag(Form("data/SVmass__ptbin_%s_DeepCSVBDisc%s_Fail", thebin, wp));

     cf->AddTemplate("b",Form("mc/SVmass__ptbin_%s_b", thebin),2);
     cf->AddTemplate("c",Form("mc/SVmass__ptbin_%s_c", thebin),3);
     cf->AddTemplate("l",Form("mc/SVmass__ptbin_%s_l", thebin),4);

     cf->AddTemplateTag("b",Form("mc/SVmass__ptbin_%s_b_DeepCSVBDisc%s", thebin, wp),2);
     cf->AddTemplateTag("c",Form("mc/SVmass__ptbin_%s_c_DeepCSVBDisc%s", thebin, wp),3);
     cf->AddTemplateTag("l",Form("mc/SVmass__ptbin_%s_l_DeepCSVBDisc%s", thebin, wp),4);

     cf->AddTemplateUntag("b",Form("mc/SVmass__ptbin_%s_b_DeepCSVBDisc%s_Fail", thebin, wp),2);
     cf->AddTemplateUntag("c",Form("mc/SVmass__ptbin_%s_c_DeepCSVBDisc%s_Fail", thebin, wp),3);
     cf->AddTemplateUntag("l",Form("mc/SVmass__ptbin_%s_l_DeepCSVBDisc%s_Fail", thebin, wp),4);   
     ...
     for (unsigned int ifit = 0; ifit < 7; ++ifit){
       ...
       if (ifit == 4) { //use MC JP calib
        cf->SetData(Form("data_inverted/SVmass__ptbin_%s", thebin));
        cf->SetDataTag(Form("data_inverted/SVmass__ptbin_%s_DeepCSVBDisc%s", thebin, wp));
        cf->SetDataUntag(Form("data_inverted/SVmass__ptbin_%s_DeepCSVBDisc%s_Fail", thebin, wp));
       } 
       if (ifit < 5){ 
         if (ifit == 4)
           system("mkdir pics");
